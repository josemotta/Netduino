...
namespace Toolbox.NETMF.Hardware
{
    ...
    public class SN754410 : IDisposable
    {
        ...
        public SN754410(Cpu.Pin Speed1, Cpu.Pin Direction1, Cpu.Pin Speed2, Cpu.Pin Direction2)
        {
            _Motor1Speed = new PWM(Speed1);
            _Motor2Speed = new PWM(Speed2);
        ...
        public void SetState(Motors Motor, sbyte Speed)
        {
            ...
            switch (Motor)
            {
                case Motors.Motor1:
                    ...
                    this._Motor1Speed.SetDutyCycle((uint)(Speed < 0 ? 0 - Speed : Speed));
                    break;
                case Motors.Motor2:
                    // Same as for Motor1
                    this._Motor2Direction.Write(Speed < 0);
                    this._Motor2Speed.SetDutyCycle((uint)(Speed < 0 ? 0 - Speed : Speed));
                    break;
                default:
                    // Should never happen!! :-)
                    throw new System.ArgumentOutOfRangeException();
        ...
        public void Dispose()
        {
            this.SetState(Motors.Motor1, 0);
            this.SetState(Motors.Motor2, 0);
            this.Dispose();
        }
    }
}
