...
namespace Toolbox.NETMF.NET
{
    ...
    public class SMTP_Client
    {
        ...
        /// <summary>Reference to the SMTP-server hostname</summary>
        private string _SMTP_Host;
        /// <summary>Reference to the SMTP-server port</summary>
        private ushort _SMTP_Port;
        ...
        /// </summary>
        /// <param name="SMTP_Host">The hostname of the SMTP-server</param>
        /// <param name="SMTP_Port">The port of the SMTP-server (default: 25)</param>
        ...
        public SMTP_Client(string SMTP_Host, ushort SMTP_Port = 25, AuthenticationTypes AuthenticationType = AuthenticationTypes.None, string Username = "", string Password = "")
        {
        ...
        private void _Send(string From, string[] Recipients, string Data)
        {
            ...
            string Text = "";
            // Creates the socket
            SimpleSocket Socket = new SimpleSocket(this._SMTP_Host, this._SMTP_Port);
            ...
            { // We will use break, also when an error occures, so we always close the connection nicely
                ...
                Text = Socket.Receive(true);
                if (Text.Substring(0, 3) != "220") { Error = Text; break; }

                // Say "hello!"
                Socket.Send("HELO " + this._LocalHostname + "\r\n");
                ...
                if (this._SMTP_Auth == AuthenticationTypes.Login)
                {
                    ...
                    Socket.Send(Tools.Base64Encode(this._SMTP_User) + "\r\n");
                    Text = Socket.Receive(true);
                    if (Text.Substring(0, 3) != "334") { Error = Text; break; }

                    // Sends the password, base64 encoded
                    Socket.Send(Tools.Base64Encode(this._SMTP_Pass) + "\r\n");
                    Text = Socket.Receive(true);
                ...
                {
                    Socket.Send("RCPT TO:<" + Recipients[Counter] + ">\r\n");
                    Text = Socket.Receive(true);
                    if (Text.Substring(0, 3) != "250") { Error = Text; break; }
                }
