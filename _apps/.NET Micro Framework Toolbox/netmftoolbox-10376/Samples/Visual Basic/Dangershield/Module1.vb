...
Module Module1
    ...
    Sub Main()
        ...
        Dim Segment7 As Bitshift7Segment = New Bitshift7Segment(New Ic74HC595Chain(Pins.GPIO_PIN_D8, Pins.GPIO_PIN_D4, Pins.GPIO_PIN_D7))
        ...
        ' Potentio meters
        Dim PotentioMeter1 As SecretLabs.NETMF.Hardware.AnalogInput = New SecretLabs.NETMF.Hardware.AnalogInput(Pins.GPIO_PIN_A0)
        PotentioMeter1.SetRange(0, 100) ' Same range as Led1.SetDutyCycle()
        Dim PotentioMeter2 As SecretLabs.NETMF.Hardware.AnalogInput = New SecretLabs.NETMF.Hardware.AnalogInput(Pins.GPIO_PIN_A1)
        PotentioMeter2.SetRange(0, 100) ' Same range as Led2.SetDutyCycle()
        Dim PotentioMeter3 As SecretLabs.NETMF.Hardware.AnalogInput = New SecretLabs.NETMF.Hardware.AnalogInput(Pins.GPIO_PIN_A2)
        PotentioMeter3.SetRange(0, 9)   ' Same range as Segment7.SetDigit()
        ' Photocell
        Dim Photocell As SecretLabs.NETMF.Hardware.AnalogInput = New SecretLabs.NETMF.Hardware.AnalogInput(Pins.GPIO_PIN_A3)
        Photocell.SetRange(0, 100) ' Same range as Led2.SetDutyCycle()
        ' Temperature Sensor
        Dim TemperatureSensor As Tmp36 = New Tmp36(Pins.GPIO_PIN_A4)
        ' Knock Sensor
        Dim KnockSensor As SecretLabs.NETMF.Hardware.AnalogInput = New SecretLabs.NETMF.Hardware.AnalogInput(Pins.GPIO_PIN_A5)
        ...
        Do
            ...
            If Demo = 1 Then
                ...
                Led1.SetDutyCycle(CUInt(PotentioMeter1.Read()))
                Led2.SetDutyCycle(CUInt(PotentioMeter2.Read()))
                Segment7.SetDigit(CByte(PotentioMeter3.Read()))
            Else
                ' Second demo is currently active
                Dim Knocking As UInteger = CUInt(KnockSensor.Read())
                If Knocking > 100 Then Led1.SetDutyCycle(100) Else Led1.SetDutyCycle(Knocking)
                Led2.SetDutyCycle(CUInt(Photocell.Read()))
